...
const postList = document.querySelector(".post-aria-list");
...
const showPost = () => {
  ...
  if(posts.length > 0){
      ...
      <div class="user-post-header">
        <div class="post-info">
          ...
          <div class="user-details">
            <a class="author" href="#"> ${data.user_name} </a>
            <span
              > ${timeAgo(data.post_time)}
              <svg
                fill="currentColor"
                viewBox="0 0 16 16"
                width="1em"
    ...
    content = `<h1>No post found yet</h1>`;
  };
...
postForm.onsubmit = (e) => {
  ...
  if (!data.user_name || !data.user_photo) {
    msg.innerHTML = creatAlert("User name and User photo both are required!");
  } else {
    // Get data from local storage
    let prevData = getData("posts");
  
    // If prevData is not an array or not defined, initialize it as an empty array
